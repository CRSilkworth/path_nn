...
This repo looks at various forms of n(d), the limit that D goes to infinity as well as the limit that W goes to infinity. As of this writing, analysis is done using mnist data but more will be added in the future.
...
```
docker run --gpus all -it -v $PWD:/tmp -w /tmp --rm path_nn/beam:latest python {mlp_subproject}/pipelines/beam/{pipeline_file}
```
